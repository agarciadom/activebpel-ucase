// $Header: /Development/AEDevelopment/projects/org.activebpel.rt.bpel/src/org/activebpel/rt/bpel/impl/AeBusinessProcessEngine.java,v 1.183 2007/08/29 15:45:3
/*
...
public class AeBusinessProcessEngine implements IAeBusinessProcessEngineInternal, IAeBusinessProcessEngineCallback
{
    ...
    * @param aAttachmentManager The attchment manager to be associated with this engine.
    */
   ...
   /**
    * @see org.activebpel.rt.bpel.IAeBusinessProcessEngine#removeVariableAttachments(long, java.lang.String, java.lang.Integer[])
    */
         ...
         if (replyReceiver != null)
         {
            synchronized(replyReceiver)
            {
               if (replyReceiver.isWaitable())
               {
                  try
                  {
                     // TODO (MF) This will be configurable by a myRoe policy
                     replyReceiver.wait(getEngineConfiguration().getWebServiceTimeout() * 1000);
                  }
            ...
            else
            {
               ...
               throw new AeTimeoutException(msg);
            }
         ...
         IAeMessageContext aContext) throws AeBusinessProcessException
   {
      ...
      Map correlationMap = createCorrelationMap(aProcessPlan, aMessageData, aContext);
      boolean isOneWay = plan.getProcessDef().isOneWayReceive(plOpKey);
      ...
      AeInboundReceive inboundReceive = new AeInboundReceive(plOpKey, correlationMap, aProcessPlan,
   ...
   protected long createProcessWithMessage(AeInboundReceive aInboundReceive, IAeMessageAcknowledgeCallback aAckCallback, boolean aQueueForExecution)
      throws AeBusinessProcessException
   {
      IAeBusinessProcess process = internalCreateProcessWithMessage(aInboundReceive, aAckCallback);
      try
      {
    ...
    * @throws Exception
    */
   protected IAeBusinessProcess internalCreateProcessWithMessage(AeInboundReceive aInboundReceive, IAeMessageAcknowledgeCallback aAckCallback) throws AeBusinessProcessException
   {
      //
      ...
      IAeBusinessProcess process = getProcessManager().createBusinessProcess( aInboundReceive.getProcessPlan() );
      long processId = process.getProcessId();
      ...
      // init proces business properties and partner link strategies.
   ...
   {      
      ...
      try
      {
         ...
         { 
            ...
            journalId = journalQueueInvokeData(aProcessId, locationId, aTransmissionId, aMessageData, aProperties, aAckCallback);
   ...
   /**
    * @see org.activebpel.rt.bpel.IAeBusinessProcessEngine#removeEngineListener(org.activebpel.rt.bpel.IAeMonitorListener)
    */
