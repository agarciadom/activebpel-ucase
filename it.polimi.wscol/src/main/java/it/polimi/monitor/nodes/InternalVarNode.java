...
public class InternalVarNode extends Variable {
	private static final Logger LOGGER = Logger.getLogger(InternalVarNode.class.getCanonicalName());
	...
	public AliasInfo getAliasValue() throws WSCoLException {
		...
		if (v instanceof String)
			...
			throw new WSCoLException("Can't create Alias");
	}
	...
	public String serializeVar() throws WSCoLException {
		...
		if (alias != null)
			if (alias.getAliasType() == Alias.ALIAS_COSTANT)
				xml = "<" + alias.serializeTag + ">"
						+ alias.getMonitoringValue() + "</"
						+ alias.serializeTag + ">";
			else
				xml = ((AliasInfo) alias.getMonitoringValue()).getCursorData()
						.xmlText();
		else if (aliasNode != null)
	...
	private String[] extractValues(String xpath) throws DataException {
		...
		int j = 0;
		LOGGER.finest("The current value is " + results.length);
		for (XmlObject i : results) {
			XmlCursor resultCursor = i.newCursor();
			res[j] = resultCursor.getTextValue();
			LOGGER.info("Extracted value " + j + " with the text " + i);
			j++;
		}
		return res;
	}
