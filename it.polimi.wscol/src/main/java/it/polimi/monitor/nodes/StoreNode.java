...
public class StoreNode extends NodeWSCoL {
        ...
        public void evaluate(InputMonitor inputMonitor, Aliases aliases, AliasNodes tempAliases) throws WSCoLException {
                ...
                if (rule instanceof Variable) {
                        if (! invokeMultipleVar(simple.getMonitoringValue(), this.extractMultivariable((Variable)rule)))
                                throw new HVarSericeException("Error in invoke Historical Variable Service");
                        /*if (! invokeService(simple.getMonitoringValue(), ((Variable)rule).serializeVar()))
                                throw new HVarSericeException("Error in invoke Historical Variable Service");*/
                } else
                        if(!invokeService(simple.getMonitoringValue(), rule.getMonitoringValue().toString()))
                                throw new HVarSericeException("Error in invoke Historical Variable Service");
        ...
        private void setConfiguration(String config) throws WSCoLException {
                ...
                if (val!=null)
                        ...
                        //errore
                        throw new IncompleteInformationException("Store web method not Found");
        ...
        /*private boolean invokeServiceAxis(String aliasName,String value) {
                try {
                        ...
                        return port.createHistoricalVariable(processID, userID, instanceID, location, assertionType, aliasName, value);
                } catch (CreateHistoricalVariableException e) {
        ...
        private String extractValue(String xpath) throws DataException {
                ...
                logger.info("il valore estratto � " +resultCursor.getTextValue());
        ...
        private boolean invokeMultipleVar(String aliasName,String[] values) throws WSCoLException {
                for(String i :values) {
        ...
        private XmlObject[] extractXML(XmlCursor cursor, String xpath) throws DataException {
                if (xpath.startsWith("/"))
                        xpath=xpath.substring(1);
                String queryExpression =  prexpath + xpath + postxpath;
