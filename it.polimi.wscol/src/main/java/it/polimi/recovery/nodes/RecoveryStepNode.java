...
public class RecoveryStepNode extends WSReLNode {
	...
	public void doRecovery(RecoveryParams recoveryParams,
		...
		if (tempRecoveryResult.isThereRebindAction()) {
			...
			recoveryResult.setNewServiceEndpoint(tempRecoveryResult
					.getNewServiceEndpoint());
		}
		// For 'substitute' or 'rebind' action used in preconditions
		if (tempRecoveryResult.isThereInokeActivity()) {
			recoveryResult.setServiceReinvocationOutput(tempRecoveryResult
		...
		// If there weren't actions needing remonitoring, the result is
		// returned.
		...
		String monitoringData = recoveryParams.getSupervisionParams()
				.getMonitoringData();
		;
		String recovery = null;
		String configHvar = recoveryParams.getSupervisionParams()
				.getConfigHvar();
		int processPriority = 0;
		int wscolRulePriority = 0;

		if (tempRecoveryResult.isThereChangingSupervisionRules()) {
			wscolRule = tempRecoveryResult.getChangeSupervisionRule()
					.getNewWSCoLRule();
			recovery = tempRecoveryResult.getChangeSupervisionRule()
					.getNewWSReLStrategy();
		} else {
		...
		if (tempRecoveryResult.isThereChangingSupervisionParams()) {
			wscolRulePriority = tempRecoveryResult.getChangeSupervisionParams()
					.getNewPriority();
		} else {
			wscolRulePriority = recoveryParams.getSupervisionParams()
					.getWscolRulePriority();
		}

		if (tempRecoveryResult.isThereChangingProcessParams()) {
			processPriority = tempRecoveryResult.getChangeProcessParams()
					.getNewPriority();
		} else {
			processPriority = recoveryParams.getProcessParams()
					.getProcessPriority();
		}

		if (tempRecoveryResult.isThereInokeActivity()) {
			// Substitute in monitoringData the new values with invoke result.
			monitoringData = this.updateMonitoringData(monitoringData,
					tempRecoveryResult.getServiceReinvocationOutput());
		}
		...
		if (tempRecoveryResult.isThereChangingProcessParams()) {
			...
			recoveryResult.setChangeProcessParams(tempRecoveryResult
					.getChangeProcessParams());
		}
		if (tempRecoveryResult.isThereChangingSupervisionParams()) {
			recoveryResult.setThereChangingSupervisionParams();
			recoveryResult.setChangeSupervisionParams(tempRecoveryResult
					.getChangeSupervisionParams());
		}
		if (tempRecoveryResult.isThereChangingSupervisionRules()) {
			recoveryResult.setThereChangingSupervisionRules();
			recoveryResult.setChangeSupervisionRule(tempRecoveryResult
		...
		if (wscolRulePriority < processPriority) {
			recoveryResult.setRecoveryResult(true);
			recoveryResult
					.addMessage("During recovery, priorities were changed and the monitoring rule is switched off now.\n");
			return;
		}
		...
		boolean result = monitor
				.evaluateMonitoring(rule, monitoringData, configHvar)
				.getValueMonitor().booleanValue();

		if (result) {
			recoveryResult.setRecoveryResult(true);
		}
		// else if(!result &&
		// tempRecoveryResult.isThereChangingSupervisionRules())
	...
	private String updateMonitoringData(String oldMonitoringData,
			String serviceInvocationResult) {
		...
		// Invoker was modified to remove all namespaces and relative prefixes,
		...
		// while((positionColon != -1) && (positionCloseBracket <
		// serviceInvocationResult.length()))
		// {
		// positionOpenBracket = serviceInvocationResult.indexOf("<",
		// positionCloseBracket);
		// positionCloseBracket = serviceInvocationResult.indexOf(">",
		// positionOpenBracket);
		// positionColon = serviceInvocationResult.indexOf(":",
		// positionOpenBracket);
		//
		// if((positionColon < positionCloseBracket) && (positionColon != -1))
		// {
		// String temp = serviceInvocationResult.substring(positionOpenBracket +
		// 1, positionColon + 1);
		//
		...
		// Retreives xpath list of parts from the Response SOAP message and
		// finds out matches with the list of variables
		// used for the monitoring. The result is a list of xpath to be replaced
		// in the monitoringData evrey loop.
		...
		if (variables.size() >= xpathList.size()) {
			...
			for (int i = 0; i < loops; i++) {
				...
				for (int l = 0; l < variables.size(); l++) {
					...
					if (bpelVariable.substring(
							bpelVariable.indexOf("/", lengthBeginXPath) + 1)
							.equals(xpath.substring(lengthBeginResponse))) {
		...
		newMonitoringData = parserBPELVariables.GetDocument().replace(
		...
		while (dollarPosition >= 0) {
			...
			if (bpelVar.contains(":")) {
				...
				while (colonPosition >= 0) {
					...
					String prefix = bpelVar.substring(slashPosition + 1,
							++colonPosition);
